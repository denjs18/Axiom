## GateXNOREntity.gd — XNOR gate. ID 4016. Output = NOT(XOR). True when even parity.
class_name GateXNOREntity
...
func phase_calculate() -> void:
	...
	for face in _get_input_faces():
		...
		if nb != null and nb.get_output(0).to_bool():
			count += 1
	emit_bool(count % 2 == 0)   # even parity = XNOR
